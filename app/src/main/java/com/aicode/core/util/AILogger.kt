...
object AILogger {
    ...
    private fun stringify(body: Any?): String = when (body) {
        null -> redactLargeMedia("null")
        is String -> redactLargeMedia(body.truncateForLog())
        else -> {
            val writer = LimitWriter(MAX_LOGGED_CHARS)
            runCatching { gson.toJson(body, writer) }.fold(
                onSuccess = { redactLargeMedia(writer.result()) },
                onFailure = { redactLargeMedia(body.toString().truncateForLog()) }
            )
        }
    }
